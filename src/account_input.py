...
endpoint = "https://eojcf9ylwovln2u.m.pipedream.net"
headers = {
...
def create_account(endpoint, headers):
    ...
    account_data = mj.build_account(val_account,
                              val_userr,
                              val_useru,
                              val_dateu,
                              val_dater,
...
randomize_button = ttk.Button(master=window, text="Random", command=randomize_account)
randomize_button.grid(row=2, column=0, padx=10, pady=5)
# submit button
submit_button = ttk.Button(master=window, text="Submit", command=lambda: create_account(endpoint, headers))
